...
public sealed class SkinMassRollupTests
{
    ...
    public async Task Calypso_316L_skin_mass_matches_manufacturer_golden()
    {
        ...
        await Assert.That(Math.Abs(mass.MassKg - GoldenMassKg)).IsLessThan(MassToleranceKg);
        await Assert.That(Math.Abs(mass.MassT - 242.948f)).IsLessThan(0.01f);
    }
    ...
    public async Task Attach_round_trips_on_cad_document()
    {
        ...
        await Assert.That(Math.Abs(mass2!.MassKg - GoldenMassKg)).IsLessThan(MassToleranceKg);
